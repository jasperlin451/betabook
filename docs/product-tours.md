...
`lib/product-tour-demo.ts` defines Alex Morgan's browser-only fixtures. Journal entries are the source for sends, projects, and analytics; analytics use the production calculation. No database demo account is needed. Negative sample IDs must never enter entity links, real forms, or actions. The only tour mutation is saving the authenticated user's dismissal/completion status. `PrivacyFields` is shared with Account: its separate Send commentary and Journal entries selectors each offer Only me, Friends, and Public, and all fields use local state callbacks in the tutorial, while `PrivacyControls` owns real saving and error handling. The privacy lesson demonstrates public commentary with a Friends-only journal. A private profile disables both selectors and shows the effective Only me audience; switching back restores the saved choices. The commentary audience also protects the mirrored original-ascent note inside a visible journal. Its visitor summary lists the audiences separately. Keep the shared fields free of actions.
