...
- Public catalog reads expose only area/subarea/route names and hierarchy. All other data APIs use `withApiSession`, and pages and metadata authorize before reading protected data. Locked pages show Sign in/Sign up callouts. Any future middleware must use the edge runtime supported by OpenNext and must not replace real authorization.
...
- Area and climb pages remain crawlable with names and hierarchy links. Their public queries use explicit field projections; metadata and JSON-LD contain only names, location trails, and generic login guidance. Never include grades, descriptions, ratings, or activity in these public payloads.
