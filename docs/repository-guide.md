...
- `pnpm test` and `pnpm check` run all three Vitest projects. Use `pnpm test:components` for jsdom, `pnpm test --project=workers` for Workers, and `pnpm test --project=scripts` for the local seed CLI against disposable migrated SQLite databases. None requires a production build or local seed. Keep the focused run's collection and result in the work summary; an uncollected test is not passing coverage.
