...
test("legacy area-name filters stay visible through refinements and can be cleared", async ({
  page,
}) => {
...
test("app search journey preserves query and category in the full-results URL", async ({
  page,
}) => {
...
test("app full search restores an explicit area ID and query with browser history", async ({
  page,
}, testInfo) => {
...
test("a new quick search session starts without previous query or category", async ({ page }) => {
  await page.goto(`${appBaseURL}/?mode=all`);
  await page.getByRole("button", { name: "Search", exact: true }).click();
  const dialog = page.getByRole("dialog", { name: "Search Betabook" });
  const input = dialog.getByRole("combobox", { name: "Search Betabook" });
...
test("full search navigation results support opening another tab", async ({ page, context }) => {
