...
for (const [story, label] of [
  ...
  test(`top navigation highlights its destination on ${story}`, async ({ page }, info) => {
    ...
    await expect(nav.getByRole("link", { name: "My Journal", exact: true })).toBeVisible();
...
test("another climber's journal does not mark My Journal current", async ({ page }, info) => {
...
test("side-menu selection fills the menu row", async ({ page }, info) => {
  ...
  const nav = page.getByRole("navigation", { name: "Primary" });
  const active = nav.getByRole("link", { name: "My Journal", exact: true });
  await expect(active).toHaveCSS("font-weight", "600");
