...
- Keep the guide in its own space beside or below the scrollable demo. Never cover controls or results with tutorial text, or dim content people need to read. Check navigation, replay, keyboard focus, mobile layout, Hide tip, and the example controls. Update tests for changed behavior and sample data, and check that demos don't change the user's account.
