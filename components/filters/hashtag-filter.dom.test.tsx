...
it.each(["{Enter}", " "])("only commits existing tags with %s and permits removal", async (key) => {
  const user = userEvent.setup();
  render(<Hashtags />);
  const input = screen.getByRole("combobox", { name: "Hashtag" });
  expect(input).toHaveValue("#");
  await user.type(input, key);
  expect(screen.queryByRole("button", { name: /^Remove hashtag/ })).not.toBeInTheDocument();
  ...
  expect(input).toHaveValue("#");
  expect(screen.getByRole("button", { name: "Remove hashtag trip" })).toHaveTextContent("#trip");
  expect(screen.queryByRole("listbox")).not.toBeInTheDocument();
  await user.type(input, `missing${key}`);
  await user.keyboard("{Escape}");
  expect(screen.getAllByRole("button", { name: /^Remove hashtag/ })).toHaveLength(1);
  await user.click(screen.getByRole("button", { name: "Remove hashtag trip" }));
  expect(screen.queryByRole("button", { name: /^Remove hashtag/ })).not.toBeInTheDocument();
});
...
it("selects suggestions, preserves the prefix, and excludes selected tags when reopened", async () => {
  ...
  expect(screen.getByRole("button", { name: "Remove hashtag outdoors" })).toBeInTheDocument();
  await user.click(input);
  ...
  expect(screen.getAllByRole("button", { name: /^Remove hashtag/ })).toHaveLength(2);
  await user.click(screen.getByRole("button", { name: "Remove hashtag outdoors" }));
  expect(screen.getByRole("button", { name: "Remove hashtag trip" })).toBeInTheDocument();
});
...
it("closes suggestions and resets the draft when the parent resets its selection", async () => {
  ...
  expect(screen.queryByRole("button", { name: "Remove hashtag trip" })).not.toBeInTheDocument();
});
